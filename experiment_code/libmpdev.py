#
# Notes from Maja 07/12/2023
#
# I got this file from Adam Culbreth
# After installing the Biopac API, use this script to load the dll
# The ctypes library allows Python to call C functions so that it works with the dll
# In line "result = mpdev.connectMPDev(c_int(101), c_int(11), b'auto')" the two numbers may need to be changed
# 101 is for mp150 and needs to be 102 or 103 for mp160 (which is what we have I think)
# 11 is for using ethernet to USB and needs to be 10 when using direct USB
# In line "set channels to acquire" you set the channels that are to be recorded from (1 records, 0 ignors)
# You could turn all of them on and then later specify which data you want to actually save, or just turn on the one you are actually using
#
# Then in the actual experiment script, to get a grip force sample, you have to write:
# from libmpdev import MP150
# mp = MP150()
# sample = mp.sample()
#
# Also of course change file paths
...
try:
	mpdev = windll.LoadLibrary('mpdev.dll')
     #C:\\Users\\Dennis Hernaus\\Desktop\\grip_exp
...
class MP150:
	...
	def __init__(self, logfile='default', samplerate=10):
		...
		try:
			result = mpdev.connectMPDev(c_int(101), c_int(11), b'auto')
                 #result = mpdev.connectMPDev(MP650, MPUDP, "auto");
		except:
